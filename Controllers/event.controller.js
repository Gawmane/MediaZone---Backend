...
class EventController {
	...
	list = async (req, res) => {
		...
		//Sender result
		res.send(result) || res.status(418).send("Kunne ikke åbne eventerne.");
	};
	...
	create = async (req, res) => {
		...
		//Hvis fejl - send fejlmeddelse
			...
			return res
				.status(418)
				.send(
					"Kunne ikke oprette eventet. Tjek om du har udfyldt alle felterne korrekt."
				);
		}
	};
	...
	update = async (req, res) => {
		...
		//Hvis fejl - send fejlmeddelse
			...
			return res
				.status(418)
				.send(
					"Kunne ikke opdatere eventet. Tjek om du har udfyldt alle felterne korrekt."
				);
		}
	};
	...
	delete = async (req, res) => {
		try {
			...
			return res
				.status(418)
				.send(
					"Kunne ikke slette eventet. Tjek om du har valid id og prøv igen."
				);
		}
	};
}
